...
<title>{{ $ekskul->nama ?? 'PMR' }} — {{ $profil->nama_sekolah ?? 'SMK Negeri 1 Cijati' }}</title>
...
<body>
  ...
  <section class="hero">
    @if($ekskul && $ekskul->foto)
      <img src="{{ asset('storage/images/'.$ekskul->foto) }}" alt="Kegiatan Ekstrakurikuler {{ $ekskul->nama ?? 'PMR' }}">
    @else
      <div class="photo-placeholder">Foto kegiatan menyusul</div>
    @endif
    <div class="hero-content">
      <div class="hero-badge">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21s-7-4.35-9.5-8.5C.5 8.5 2.5 5 6 5c2 0 3.5 1.5 4 2.5.5-1 2-2.5 4-2.5 3.5 0 5.5 3.5 3.5 7.5C19 16.65 12 21 12 21z"/><path d="M12 8v6"/><path d="M9 11h6"/></svg>
      </div>
      <span class="hero-tag">Ekstrakurikuler · Palang Merah Remaja</span>
      <h1>{{ $ekskul->nama ?? 'PMR' }}</h1>
      <p>Membekali kesiapsiagaan pertolongan pertama, kesehatan, dan kepedulian sosial.</p>
    </div>
  </section>
    ...
    <div class="card about-card">
      <div class="about-photo">
        @if($ekskul && $ekskul->pembina && $ekskul->pembina->foto)
          <img src="{{ asset('storage/images/guru-guru/'.rawurlencode($ekskul->pembina->foto)) }}" alt="{{ $ekskul->pembina->nama }} - Pembina {{ $ekskul->nama ?? 'PMR' }}">
        @else
          <div class="foto-kosong">Foto pembina menyusul</div>
        @endif
        <span class="caption">
          {{ $ekskul->pembina->nama ?? 'Pembina belum diisi' }}
          <small>Pembina {{ $ekskul->nama ?? 'PMR' }}</small>
        </span>
      </div>
      <div class="txt">
        <span class="eyebrow">Tentang</span>
        <h2>Ekstrakurikuler {{ $ekskul->nama ?? 'PMR' }}</h2>
    ...
    <div class="card">
      ...
      <div class="gallery">
        ...
        @forelse($fotoDokumentasi as $foto)
          <img src="{{ asset('storage/images/'.$foto) }}" alt="Dokumentasi {{ $ekskul->nama ?? 'PMR' }}">
        @empty
  ...
  <footer>
    &copy; {{ date('Y') }} {{ $profil->nama_sekolah ?? 'SMK Negeri 1 Cijati' }} — Ekstrakurikuler {{ $ekskul->nama ?? 'PMR' }}
  </footer>
