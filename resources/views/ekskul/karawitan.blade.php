...
<title>{{ $ekskul->nama ?? 'Karawitan' }} — {{ $profil->nama_sekolah ?? 'SMK Negeri 1 Cijati' }}</title>
...
<body>
  ...
  <section class="hero">
    @if($ekskul && $ekskul->foto)
      <img src="{{ asset('storage/images/'.$ekskul->foto) }}" alt="Kegiatan Ekstrakurikuler {{ $ekskul->nama ?? 'Karawitan' }}">
    @else
      <div class="photo-placeholder">Foto kegiatan menyusul</div>
    @endif
    <div class="hero-content">
      <div class="hero-badge">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M9 8v8"/><path d="M12 8v8"/><path d="M15 8v8"/></svg>
      </div>
      <span class="hero-tag">Ekstrakurikuler · Seni Musik Tradisional</span>
      <h1>{{ $ekskul->nama ?? 'Karawitan' }}</h1>
      <p>Melestarikan seni musik tradisional gamelan dan menumbuhkan kecintaan pada budaya lokal.</p>
    </div>
  </section>
    ...
    <div class="card about-card">
      <div class="about-photo">
        @if($ekskul && $ekskul->pembina && $ekskul->pembina->foto)
          <img src="{{ asset('storage/images/guru-guru/'.rawurlencode($ekskul->pembina->foto)) }}" alt="{{ $ekskul->pembina->nama }} - Pembina {{ $ekskul->nama ?? 'Karawitan' }}">
        @else
          <div class="foto-kosong">Foto pembina menyusul</div>
        @endif
        <span class="caption">
          {{ $ekskul->pembina->nama ?? 'Pembina belum diisi' }}
          <small>Pembina {{ $ekskul->nama ?? 'Karawitan' }}</small>
        </span>
      </div>
      <div class="txt">
        <span class="eyebrow">Tentang</span>
        <h2>Ekstrakurikuler {{ $ekskul->nama ?? 'Karawitan' }}</h2>
    ...
    <div class="card">
      ...
      <div class="gallery">
        ...
        @forelse($fotoDokumentasi as $foto)
          <img src="{{ asset('storage/images/'.$foto) }}" alt="Dokumentasi {{ $ekskul->nama ?? 'Karawitan' }}">
        @empty
  ...
  <footer>
    &copy; {{ date('Y') }} {{ $profil->nama_sekolah ?? 'SMK Negeri 1 Cijati' }} — Ekstrakurikuler {{ $ekskul->nama ?? 'Karawitan' }}
  </footer>
