...
<title>{{ $ekskul->nama ?? 'Rohis' }} — {{ $profil->nama_sekolah ?? 'SMK Negeri 1 Cijati' }}</title>
...
<body>
  ...
  <section class="hero">
    @if($ekskul && $ekskul->foto)
      <img src="{{ asset('storage/images/'.$ekskul->foto) }}" alt="Kegiatan Ekstrakurikuler {{ $ekskul->nama ?? 'Rohis' }}">
    @else
    ...
    <div class="hero-content">
      ...
      <span class="hero-tag">Ekstrakurikuler · Rohani Islam</span>
      <h1>{{ $ekskul->nama ?? 'Rohis' }}</h1>
      <p>Mempererat keimanan dan ketakwaan lewat kajian, tadarus, dan kepedulian sosial keagamaan.</p>
    </div>
  </section>
    ...
    <div class="card about-card">
      <div class="about-photo">
        @if($ekskul && $ekskul->pembina && $ekskul->pembina->foto)
          <img src="{{ asset('storage/images/guru-guru/'.rawurlencode($ekskul->pembina->foto)) }}" alt="{{ $ekskul->pembina->nama }} - Pembina {{ $ekskul->nama ?? 'Rohis' }}">
        @else
          <div class="foto-kosong">Foto pembina menyusul</div>
        @endif
        <span class="caption">
          {{ $ekskul->pembina->nama ?? 'Pembina belum diisi' }}
          <small>Pembina {{ $ekskul->nama ?? 'Rohis' }}</small>
        </span>
      </div>
      <div class="txt">
        <span class="eyebrow">Tentang</span>
        <h2>Ekstrakurikuler {{ $ekskul->nama ?? 'Rohis' }}</h2>
    ...
    <div class="card">
      ...
      <div class="gallery">
        ...
        @forelse($fotoDokumentasi as $foto)
          <img src="{{ asset('storage/images/'.$foto) }}" alt="Dokumentasi {{ $ekskul->nama ?? 'Rohis' }}">
        @empty
  ...
  <footer>
    &copy; {{ date('Y') }} {{ $profil->nama_sekolah ?? 'SMK Negeri 1 Cijati' }} — Ekstrakurikuler {{ $ekskul->nama ?? 'Rohis' }}
  </footer>
