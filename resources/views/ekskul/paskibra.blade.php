...
<title>{{ $ekskul->nama ?? 'Paskibra' }} — {{ $profil->nama_sekolah ?? 'SMK Negeri 1 Cijati' }}</title>
...
<body>
  ...
  <section class="hero">
    @if($ekskul && $ekskul->foto)
      <img src="{{ asset('storage/images/'.$ekskul->foto) }}" alt="Kegiatan Ekstrakurikuler {{ $ekskul->nama ?? 'Paskibra' }}">
    @else
      <div class="photo-placeholder">Foto kegiatan menyusul</div>
    @endif
    <div class="hero-content">
      <div class="hero-badge">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 21V4a1 1 0 0 1 1-1h1"/><path d="M6 3v6l7-3 7 3V3"/><path d="M13 6v15"/></svg>
      </div>
      <span class="hero-tag">Ekstrakurikuler · Baris-Berbaris</span>
      <h1>{{ $ekskul->nama ?? 'Paskibra' }}</h1>
      <p>Melatih kedisiplinan, ketegasan sikap, dan kekompakan formasi baris-berbaris.</p>
    </div>
  </section>
    ...
    <div class="card about-card">
      <div class="about-photo">
        @if($ekskul && $ekskul->pembina && $ekskul->pembina->foto)
          <img src="{{ asset('storage/images/guru-guru/'.rawurlencode($ekskul->pembina->foto)) }}" alt="{{ $ekskul->pembina->nama }} - Pembina {{ $ekskul->nama ?? 'Paskibra' }}">
        @else
          <div class="foto-kosong">Foto pembina menyusul</div>
        @endif
        <span class="caption">
          {{ $ekskul->pembina->nama ?? 'Pembina belum diisi' }}
          <small>Pembina {{ $ekskul->nama ?? 'Paskibra' }}</small>
        </span>
      </div>
      <div class="txt">
        <span class="eyebrow">Tentang</span>
        <h2>Ekstrakurikuler {{ $ekskul->nama ?? 'Paskibra' }}</h2>
    ...
    <div class="card">
      ...
      <div class="gallery">
        ...
        @forelse($fotoDokumentasi as $foto)
          <img src="{{ asset('storage/images/'.$foto) }}" alt="Dokumentasi {{ $ekskul->nama ?? 'Paskibra' }}">
        @empty
  ...
  <footer>
    &copy; {{ date('Y') }} {{ $profil->nama_sekolah ?? 'SMK Negeri 1 Cijati' }} — Ekstrakurikuler {{ $ekskul->nama ?? 'Paskibra' }}
  </footer>
