...
<title>{{ $ekskul->nama ?? 'Voli Bal' }} — {{ $profil->nama_sekolah ?? 'SMK Negeri 1 Cijati' }}</title>
...
<body>
  ...
  <section class="hero">
    @if($ekskul && $ekskul->foto)
      <img src="{{ asset('storage/images/'.$ekskul->foto) }}" alt="Kegiatan Ekstrakurikuler {{ $ekskul->nama ?? 'Voli Bal' }}">
    @else
      <div class="photo-placeholder">Foto kegiatan menyusul</div>
    @endif
    <div class="hero-content">
      <div class="hero-badge">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M12 3c2.5 2.5 2.5 15.5 0 18"/><path d="M12 3c-2.5 2.5-2.5 15.5 0 18"/><path d="M3 12h18"/></svg>
      </div>
      <span class="hero-tag">Ekstrakurikuler · Bola Voli</span>
      <h1>{{ $ekskul->nama ?? 'Voli Bal' }}</h1>
      <p>Membangun kekompakan tim lewat servis, smash, dan blocking — rutin berlatih dan bertanding antar sekolah.</p>
    </div>
  </section>
    ...
    <div class="card about-card">
      <div class="about-photo">
        @if($ekskul && $ekskul->pembina && $ekskul->pembina->foto)
          <img src="{{ asset('storage/images/guru-guru/'.rawurlencode($ekskul->pembina->foto)) }}" alt="{{ $ekskul->pembina->nama }} - Pembina {{ $ekskul->nama ?? 'Voli Bal' }}">
        @else
          <div class="foto-kosong">Foto pembina menyusul</div>
        @endif
        <span class="caption">
          {{ $ekskul->pembina->nama ?? 'Pembina belum diisi' }}
          <small>Pembina {{ $ekskul->nama ?? 'Voli Bal' }}</small>
        </span>
      </div>
      <div class="txt">
        <span class="eyebrow">Tentang</span>
        <h2>Ekstrakurikuler {{ $ekskul->nama ?? 'Voli Bal' }}</h2>
    ...
    <div class="card">
      ...
      <div class="gallery">
        ...
        @forelse($fotoDokumentasi as $foto)
          <img src="{{ asset('storage/images/'.$foto) }}" alt="Dokumentasi {{ $ekskul->nama ?? 'Voli Bal' }}">
        @empty
  ...
  <footer>
    &copy; {{ date('Y') }} {{ $profil->nama_sekolah ?? 'SMK Negeri 1 Cijati' }} — Ekstrakurikuler {{ $ekskul->nama ?? 'Voli Bal' }}
  </footer>
